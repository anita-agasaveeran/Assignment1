"""Python port of the dataviz skill's validate_palette.js (no JS runtime on this box).

Same thresholds, same Machado-Oliveira-Fernandes (2009) severity-1.0 CVD
transforms, same OKLab Delta E x100 metric. Kept in lockstep with the JS twin.
"""
...
def rel_lum(h): r, g, b = lin(h); return 0.2126*r + 0.7152*g + 0.0722*b
def contrast(a, b):
...
def validate(palette, mode="light", surface=None, pairs="adjacent"):
    ...
    state = "pass" if wd >= CVD_TARGET else ("WARN(floor)" if wd >= CVD_FLOOR else "FAIL")
    ...
    report.append(("Contrast vs surface", "relief" if low else "pass",
                   f"below {CONTRAST_MIN}:1 - relief required (visible labels/table view): {low}"
                   if low else f"all {len(palette)} >= {CONTRAST_MIN}:1"))
    return ok, report

if __name__ == "__main__":
    pal = [c.strip() for c in sys.argv[1].split(",") if c.strip()]
    mode = "light"; pairs = "adjacent"; surface = None
    args = sys.argv[2:]
    for i, a in enumerate(args):
        if a == "--mode": mode = args[i+1]
        if a == "--pairs": pairs = args[i+1]
        if a == "--surface": surface = args[i+1]
    ok, rep = validate(pal, mode, surface, pairs)
    print(f"palette={len(pal)} mode={mode} pairs={pairs} surface={surface or DEFAULT_SURFACE[mode]}")
    for name, state, detail in rep:
        print(f"  {state:12s} {name:22s} {detail}")
